...
def run(args: Namespace) -> None:
    ...
    if payment["status"] != "SETTLED":
        print(f"Payment must be SETTLED before delivery: {payment['status']}")
        return
    ...
    print(f"STEP2 delivered payment: {payment_id}")


def register(subparsers: _SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "step2-deliver", help="Deliver a settled SEPA payment."
    )
